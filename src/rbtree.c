...
node_t *rbtree_insert(rbtree *t, const key_t key) {
  ...
  while(x != t->nil){
    ...
    y = x;
    if(z->key < x->key){
      x = x->left;      // x에 x의 왼쪽 자식 노드 저장
    }else{
      x = x->right;     // x에 x의 오른쪽 자식 노드 저장
    }
  }
  z->parent = y;      // z.p(z의 부모노드)를 y로 변경
  if (y==t->nil){
    t->root = z;
  }else if(z->key < y->key){      // z의 key값 < y의 키값
    y->left = z;      // y의 왼쪽 자식 노드를 z로 변경
  }else{
    y->right = z;     // y의 오른쪽 자식 노드를 z로 변경
  }
  z->left = t->nil;       // RB tree의 구조 유지를 위해
  z->right = t->nil;      // z의 자식 노드를 T.nil로 변경
  z->color = RBTREE_RED;    // 새로 삽입되는 노드 색 = RED

  // RB-INSERT-FIXUP 
  while (z->parent->color == RBTREE_RED){
    if(z->parent == z->parent->parent->left)    // new node의 부모노드가 왼쪽 자식인 경우
    {
      y = z->parent->parent->right;             // y는 부모 노드의 형제 노드
      ...
      y = z->parent->parent->left;        // 삼촌 노드 정의
